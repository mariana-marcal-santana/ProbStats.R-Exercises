...
eq1 <- function(x) pchisq(x[2], df = n - 1) - pchisq(x[1], df = n - 1) - gamma
eq2 <- function(x) dchisq(x[2], df = n + 3) - dchisq(x[1], df = n + 3)

sol <- fsolve(x0 = c(a, b), eq1, eq2)

new_interval <- c(((n - 1) * var_sample) / sol[2], ((n - 1) * var_sample) / sol[1])
